...
def plot_errors(x, train_err, test_err):
	...
	plt.plot(x, test_err, marker='o', linestyle='..', color='r', label='Test Error')
	plt.xlabel('Epochs')
...
for i in xrange(iterations):
	...
	if i%100 == 0 or i==iterations-1:
		...
		test_error_data.append(percentError(two_pred_y, two_test_y))
plot_errors(epochs, train_error_data, test_error_data)
...
generate_report(bdt_discrete, one_pred_y, one_test_y)
print "*"*50
...
generate_report(svc, one_pred_y, one_test_y)
print "*"*50
...
generate_report(neigh, one_pred_y, one_test_y)
